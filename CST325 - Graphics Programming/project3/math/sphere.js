 ...
 * Required Text: Our code is very similar bc we worked together via zoom, but I believe we have minor differences
 * 
 * 
 * 
...
Sphere.prototype = {
  ...
  var result = {
    ...
    // || ray - center || - radius
    // ray - origin + scalar
    // || [origin + scalar] - center || - r = 0

    //a b and c for quad equation
    let a = r1.direction.dot(r1.direction);
    let b = 2 * r1.direction.dot(r1.origin.clone().subtract(this.center));
    let c =  r1.origin.clone().subtract(this.center).dot(r1.origin.clone().subtract(this.center))  - Math.pow(this.radius,2);

    let partialQuadratic = Math.pow(b,2) - (4 * a * c); // make this into var since it doesn't change in either formula
  
    let quadratic1 = ((-b) + Math.sqrt(partialQuadratic)) / (2*a);
    let quadratic2 = ((-b) - Math.sqrt(partialQuadratic)) / (2*a);


    //checks for no hits
    if(partialQuadratic < 0){
      result.hit = false;
      result.point = null;
      return result;
    }

    if(quadratic1 < 1 || quadratic2 < 1){ //done to make sure one of the alphas isn't negative (behind ray)
      result.hit = false;
      result.point = null;
      return result;
    }
    ...
    if (quadratic1 < quadratic2){
      result.hit = true;
      result.point = r1.origin.clone().add(r1.direction.clone().multiplyScalar(quadratic1));
      result.normal =  r1.origin.clone().add(r1.direction.clone().multiplyScalar(quadratic1)).clone().subtract(this.center).normalize(),
      result.distance = r1.direction.clone().multiplyScalar(quadratic1).length();
      return result;
    } else {
      result.hit = true;
      result.point = r1.origin.clone().add(r1.direction.clone().multiplyScalar(quadratic2));
      result.normal =  r1.origin.clone().add(r1.direction.clone().multiplyScalar(quadratic2)).clone().subtract(this.center).normalize(),
      result.distance = r1.direction.clone().multiplyScalar(quadratic2).length();
      return result;
    }
