...
function createScene() {
    ...
    let groundSize = new Matrix4().makeScale(10,1,10);
    let groundRotation = new Matrix4().makeRotationX(90);
    let groundRotationAndScaling = groundSize.multiply(groundRotation);
    groundGeometry.worldMatrix.multiply(groundRotationAndScaling);
...
function updateAndRender() {
    ...
    // render ground
    gl.uniform4f(colorProgram.uniforms.colorUniform, 0.4, 0.5, 0.4, 1.0);
    ...
    let sinFunction = Math.abs(Math.sin(time.secondsElapsedSinceStart));
    // todo #10 - animate the color with non-grayscale values

    // todo #3 - render the sphere
    gl.uniform4f(colorProgram.uniforms.colorUniform,sinFunction, sinFunction*sinFunction*sinFunction, 1 - ((1 - sinFunction) * sinFunction),1.0);
