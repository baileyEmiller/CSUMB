...
function updateAndRender() {
    ...
    if(appInput.w){ //x-1
        rotMatrix.makeRotationX(1);
        var changes = new Vector3(rotMatrix.elements[0], rotMatrix.elements[6],rotMatrix.elements[10]).normalize();
        lightDirection.subtract(changes);
    } else if (appInput.d){ //y1
        rotMatrix.makeRotationY(1);
        var changes = new Vector3(rotMatrix.elements[0], rotMatrix.elements[6],rotMatrix.elements[10]).normalize();
        lightDirection.add(changes);
    } else if (appInput.s) { //x1
        rotMatrix.makeRotationX(1);
        var changes = new Vector3(rotMatrix.elements[0], rotMatrix.elements[6],rotMatrix.elements[10]).normalize();
        lightDirection.add(changes);
    } else if (appInput.a){ //y-1
        rotMatrix.makeRotationY(1);
        var changes = new Vector3(rotMatrix.elements[0], rotMatrix.elements[6],rotMatrix.elements[10]).normalize();
        lightDirection.substract(changes);
    }
